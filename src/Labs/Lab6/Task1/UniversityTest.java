...
class University {
    ...
    public Map<Integer, List<Course>> groupByDifficulty() {
        ...
        return departments.stream()
                .flatMap(d -> d.getCourses().stream())
                .collect(Collectors
                        .groupingBy(Course::getDifficulty));
    }
    ...
    public University mergeFourSmallestDepartments() {
        ...
        List<Department> smallestFour = sorted.stream().limit(4).collect(Collectors.toList());
        ...
        newList.add(merged);
        newList.addAll(sorted.subList(4, sorted.size()));
        return new University(newList);
    }
}
